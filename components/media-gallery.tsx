...
export function MediaGallery() {
  ...
  return (
    <section className="relative isolate overflow-hidden bg-[#eee3d6] px-4 pb-24 pt-14 text-[#4e1b1e] sm:px-8">
      <svg
        className="pointer-events-none absolute -left-16 -top-36 z-0 h-96 w-[520px] text-[#977d69]/28"
        viewBox="0 0 520 380"
        fill="none"
        aria-hidden="true"
      >
        ...
        <div className="mb-10 flex flex-col items-start gap-6 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="group inline-flex h-14 items-center gap-2.5 rounded-full bg-[#4e1b1e] pl-7 pr-6 text-[14px] font-semibold uppercase tracking-[0.12em] text-[#f9eee1] shadow-[0_14px_34px_rgba(78, 27, 30,0.22)] transition-transform duration-300 hover:scale-[1.04]"
            >
              {brand.media.ctaLabel}
              <span className="grid h-7 w-7 place-items-center rounded-full bg-[#f9eee1]/18 transition-transform duration-300 group-hover:translate-x-0.5">
          ...
          {items.map((image) => {
            ...
            return (
              <div
                key={image.src}
                data-media-card
                className="relative aspect-[3/4] w-[80%] shrink-0 snap-start overflow-hidden rounded-[26px] bg-[#e2d0b9] shadow-[0_20px_54px_rgba(78, 27, 30,0.16)] sm:w-[46%] lg:w-[calc((100%-2.5rem)/3)]"
              >
                {isVideo ? (
                  <video
                    src={image.src}
...
function CarouselButton({
  ...
  return (
    <button
      ...
      className="grid h-14 w-14 place-items-center rounded-full border border-[#4e1b1e]/18 bg-[#f9eee1] text-[#4e1b1e] transition-all duration-300 hover:border-[#4e1b1e] hover:bg-[#4e1b1e] hover:text-[#f9eee1] disabled:cursor-not-allowed disabled:opacity-35 disabled:hover:border-[#4e1b1e]/18 disabled:hover:bg-[#f9eee1] disabled:hover:text-[#4e1b1e]"
    >
