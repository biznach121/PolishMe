    ...
    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1.4fr]">
      ...
      <div className="spabox-card flex flex-col p-5 sm:p-6">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            ...
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#4e1b1e]/14 bg-[#f9eee1] px-4 text-sm font-semibold text-[#4e1b1e] transition-colors hover:border-[#4e1b1e]/40"
          ...
          >
            ...
            Filters
            {activeFilterCount > 0 && (
              <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#4e1b1e] px-1.5 text-[11px] font-bold text-[#f9eee1]">
                {activeFilterCount}
              </span>
            )}
          </button>
        ...
        <label className="relative mb-3 block">
          ...
          <SearchIcon className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-[#947247]" />
          <input
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={searchPlaceholder}
            className="h-11 w-full rounded-full border border-[#4e1b1e]/14 bg-[#f9eee1] pl-12 pr-4 text-sm font-medium text-[#4e1b1e] outline-none transition-colors placeholder:text-[#947247] focus:border-[#4e1b1e]/40"
          />
        </label>
        ...
        <p className="mb-2 text-xs font-medium text-[#5b2d2d]/68">
        ...
        <div className="no-scrollbar max-h-[420px] space-y-2 overflow-y-auto lg:max-h-[560px]">
          {filtered.length === 0 ? (
            <div className="rounded-[18px] border border-dashed border-[#4e1b1e]/20 p-6 text-center">
              <p className="m-0 text-sm font-medium text-[#5b2d2d]/72">No treatments match.</p>
              <button
                type="button"
                onClick={() => {
                  clearAll();
                ...
                className="mt-2 text-sm font-semibold text-[#947247] underline-offset-4 hover:underline"
              >
                Reset
              </button>
            </div>
                ...
                <button
                  ...
                  className={[
                    ...
                    active
                      ...
                      : "border-[#4e1b1e]/12 bg-[#f9eee1]/68 text-[#4e1b1e] hover:border-[#4e1b1e]/35",
                  ].join(" ")}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      <p className="m-0 truncate text-sm font-semibold">{treatment.name}</p>
                      <p className={`m-0 mt-0.5 text-xs ${active ? "text-[#f9eee1]/72" : "text-[#5b2d2d]/68"}`}>
                        {treatment.duration_minutes ? `${treatment.duration_minutes} min · ` : ""}
                        {brand.currency} {treatment.default_price}
                      </p>
                    </div>
                    {active && (
                      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-[#f9eee1] text-xs text-[#4e1b1e]">
                        ✓
                      </span>
                    )}
                    {treatment.image_url && (
      ...
      <div className="spabox-card p-4 sm:p-6">
        ...
        <button
          ...
          className="mt-6 inline-flex w-full items-center justify-center gap-2 bg-[#4e1b1e] px-5 py-3 text-sm font-semibold text-[#f9eee1] transition-colors hover:bg-[#391517] disabled:cursor-not-allowed disabled:opacity-50"
        >
          {submitting
            ? "Confirming…"
            : selectedSlot
